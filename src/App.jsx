...
import { fetchResults, insertResult, updateResult, deleteResult, fetchSettings, upsertSettings } from "./supabase.js";
...
const INITIAL_RESULTS = [
  { id: 1, date: "11 Apr 2026", opposition: "Hebburn Town Gold Madrid", homeScore: 3, awayScore: 0, scorers: ["Grayson ×2", "Clayton"], result: "W", competition: "Spring Cup", motm: "Grayson", oppMotm: "" },
  { id: 2, date: "18 Apr 2026", opposition: "Hetton Juniors Sampdoria", homeScore: 2, awayScore: 0, scorers: ["Clayton ×2"], result: "W", competition: "Spring Cup", motm: "Clayton", oppMotm: "" },
  { id: 3, date: "2 May 2026", opposition: "Sunderland Girls", homeScore: 0, awayScore: 3, scorers: [], result: "L", competition: "Spring Cup", motm: "", oppMotm: "" },
  { id: 4, date: "9 May 2026", opposition: "Whitburn & Cleadon Madrid", homeScore: 2, awayScore: 0, scorers: ["Grayson", "Reggie"], result: "W", competition: "Spring Cup", motm: "Reggie", oppMotm: "" },
  { id: 5, date: "30 May 2026", opposition: "Chester Le Street Waldridge", homeScore: 6, awayScore: 2, scorers: ["Lewiee", "Reggie", "Grayson", "Kayson ×3"], result: "W", competition: "Spring Cup", motm: "Kayson", oppMotm: "" },
  { id: 6, date: "4 Jun 2026", opposition: "Washington Tigons", homeScore: 3, awayScore: 2, scorers: ["Grayson ×2", "Kayson"], result: "W", competition: "Spring Cup", motm: "Grayson", oppMotm: "" },
  { id: 7, date: "6 Jun 2026", opposition: "Deerness Valley", homeScore: 9, awayScore: 3, scorers: ["Henry", "Reggie ×2", "Grayson ×5", "Archie"], result: "W", competition: "Spring Cup", motm: "Grayson", oppMotm: "" },
];
      ...
      {data.length === 0
        ...
        : (
          <div style={{ background: "#fff", borderRadius: 16, overflow: "hidden", boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
            <div style={{ display: "grid", gridTemplateColumns: "36px 1fr 120px 48px", padding: "10px 18px", background: "#f7f8fa", borderBottom: "2px solid #e8e8e8" }}>
              {["#", "NAME", "", ""].map((h, i) => <span key={i} style={{ fontSize: 10, fontWeight: 800, color: "#aaa", letterSpacing: 2 }}>{h}</span>)}
            </div>
            {data.map((p, i) => (
              <div key={p.name} style={{ display: "grid", gridTemplateColumns: "36px 1fr 120px 48px", alignItems: "center", padding: "13px 18px", borderBottom: i < data.length - 1 ? "1px solid #f0f0f0" : "none", background: i === 0 ? "#fffbea" : "#fff" }}>
...
function SaveCardButton({ cardRef, filename = "leon-result.png" }) {
  ...
  return (
    <div style={{ marginTop: 12 }}>
      ...
      {error && (
        <p style={{ textAlign: "center", color: "#d50000", fontSize: 12, marginTop: 8, fontFamily: "inherit" }}>
          Couldn't save — try taking a screenshot instead.
        </p>
      )}
    </div>
  );
...
function ResultCard({ match, oppLogo, teamName = "Under 9 Blue", compColor = "#87ceeb" }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "24px 20px 16px", gap: 12 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1.2, gap: 8 }}>
          <div style={{ background: "#f0f4ff", borderRadius: 14, padding: 10, display: "flex", alignItems: "center", justifyContent: "center", width: 80, height: 80 }}>
            {oppLogo ? <img src={oppLogo} alt={match.opposition} style={{ width: 68, height: 68, objectFit: "contain" }} /> : <span style={{ fontSize: 30 }}>⚽</span>}
          </div>
...
export default function App() {
  ...
  // Load everything from Supabase on first open
  useEffect(() => {
    async function load() {
      try {
        const [rows, settings] = await Promise.all([fetchResults(), fetchSettings()]);
        setResults(rows || []);
        if (settings) {
          if (settings.team_name) { setTeamName(settings.team_name); setTempTeamName(settings.team_name); }
          if (settings.competitions?.length) {
            setCompetitions(settings.competitions);
            setForm(f => ({ ...f, competition: settings.competitions[0] }));
          }
        }
      } catch (e) {
        setDbError(true);
  ...
  // Save settings whenever teamName or competitions change (debounced)
  useEffect(() => {
    if (loading) return;
    const t = setTimeout(() => {
      upsertSettings({ team_name: teamName, competitions }).catch(() => {});
    }, 800);
    return () => clearTimeout(t);
  }, [teamName, competitions, loading]);
  ...
  const handleLogoUpload = (e) => {
    ...
    reader.onload = (ev) => setOppLogo(ev.target.result);
  ...
  const handleAddComp = () => {
    ...
    if (name && !competitions.includes(name)) { setCompetitions(prev => [...prev, name]); setForm(f => ({ ...f, competition: name })); }
    setNewCompName(""); setAddingComp(false);
  };

  const handleRenameComp = (idx) => {
    const oldName = competitions[idx];
    const newName = tempCompName.trim();
    if (!newName || newName === oldName) { setEditingComp(null); return; }
    if (competitions.includes(newName)) { alert("A competition with that name already exists."); return; }
    ...
    if (form.competition === oldName) setForm(f => ({ ...f, competition: newName }));
    setEditingComp(null);
  };
  ...
  const handleSaveEdit = async () => {
    ...
    const result = hs > as_ ? "W" : hs < as_ ? "L" : "D";
    const scorerList = typeof editingResult.scorers === "string"
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#1a1a2e", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20, fontFamily: "'Barlow Condensed','Arial Narrow',Arial,sans-serif" }}>
      <LeonLogo size={80} />
  ...
  if (dbError) return (
    <div style={{ minHeight: "100vh", background: "#1a1a2e", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, padding: 24, fontFamily: "'Barlow Condensed','Arial Narrow',Arial,sans-serif", textAlign: "center" }}>
      <LeonLogo size={80} />
      <div style={{ color: "#d50000", fontSize: 22, fontWeight: 800, letterSpacing: 2 }}>⚠️ CONNECTION ERROR</div>
      <div style={{ color: "#aaa", fontSize: 15, maxWidth: 320, lineHeight: 1.5 }}>
        Could not connect to the database. Check your Supabase URL and API key in <code style={{ color: "#87ceeb" }}>src/supabase.js</code>
      </div>
      <button onClick={() => window.location.reload()}
        style={{ background: "#87ceeb", color: "#1a1a2e", border: "none", borderRadius: 10, padding: "12px 24px", fontWeight: 800, fontSize: 15, cursor: "pointer", fontFamily: "inherit", letterSpacing: 1 }}>
        Try Again
      </button>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f0f2f5", fontFamily: "'Barlow Condensed','Arial Narrow',Arial,sans-serif", paddingBottom: 40 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {editingTeamName ? (
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
              ...
              <button onClick={() => { setTeamName(tempTeamName); setEditingTeamName(false); }}
                style={{ background: "#87ceeb", border: "none", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontWeight: 800, fontSize: 12, color: "#1a1a2e", fontFamily: "inherit" }}>Save</button>
            </div>
          ) : (
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 2 }}>
              <div style={{ color: "#87ceeb", fontWeight: 700, fontSize: 13, letterSpacing: 3, textTransform: "uppercase" }}>{teamName}</div>
              <button onClick={() => { setTempTeamName(teamName); setEditingTeamName(true); }}
                style={{ background: "rgba(135,206,235,0.2)", border: "1px solid rgba(135,206,235,0.4)", borderRadius: 5, padding: "2px 8px", cursor: "pointer", color: "#87ceeb", fontSize: 10, fontWeight: 700, fontFamily: "inherit", letterSpacing: 1 }}>✏️ EDIT</button>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Tab nav */}
      <div style={{ display: "flex", background: "#fff", borderBottom: "2px solid #e8e8e8" }}>
        {[{ key: "history", label: "📋 Results" }, { key: "scorers", label: "⭐ Awards" }, { key: "new", label: "➕ New" }].map(tab => (
          <button key={tab.key} onClick={() => { setMode(tab.key); setNewResult(null); setOppLogo(null); }}
            style={{ flex: 1, padding: "14px 0", border: "none", background: "none", cursor: "pointer", fontFamily: "inherit", fontWeight: 800, fontSize: 13, letterSpacing: 1, textTransform: "uppercase", color: mode === tab.key ? "#1a1a2e" : "#999", borderBottom: mode === tab.key ? "3px solid #87ceeb" : "3px solid transparent", transition: "all 0.15s" }}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      {(mode === "history" || mode === "scorers") && (
        <div style={{ background: "#fff", borderBottom: "1px solid #f0f0f0", padding: "10px 16px", display: "flex", gap: 8, overflowX: "auto", alignItems: "center" }}>
          ...
          {["All", ...competitions].map((comp, idx) => {
            ...
            const compIdx = idx - 1; // real index in competitions array
            return (
                ...
                {editingComp === compIdx && !isAll ? (
                  ...
                  <>
                    <button onClick={() => { setFilterComp(comp); setSelectedMatch(null); }}
                      style={{ padding: "6px 12px", borderRadius: 20, border: active ? "none" : "1.5px solid #e0e0e0", background: active ? color : "#fff", color: active ? (isAll ? "#87ceeb" : "#1a1a2e") : "#888", fontWeight: 800, fontSize: 12, letterSpacing: 1, cursor: "pointer", whiteSpace: "nowrap", fontFamily: "inherit", textTransform: "uppercase", transition: "all 0.15s" }}>
                      {comp}
                    </button>
                    {!isAll && (
                      <button onClick={() => { setEditingComp(compIdx); setTempCompName(comp); }}
                        title="Rename competition"
                        style={{ background: "none", border: "none", cursor: "pointer", fontSize: 11, color: "#bbb", padding: "2px 2px", lineHeight: 1 }}>✏️</button>
                    )}
                  </>
                )}
              </div>
          ...
          <div>
            <div style={{ display: "flex", gap: 10, justifyContent: "center", marginBottom: 20, flexWrap: "wrap" }}>
              {[
                ...
                { label: "Goals", val: filteredResults.reduce((a, r) => a + r.homeScore, 0), color: "#87ceeb" },
              ].map(s => (
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", maxWidth: 520, margin: "0 auto 12px" }}>
              <button onClick={() => setSortOrder(s => s === "desc" ? "asc" : "desc")}
                style={{ background: "#fff", border: "1.5px solid #e0e0e0", borderRadius: 20, padding: "6px 14px", cursor: "pointer", fontFamily: "inherit", fontWeight: 800, fontSize: 12, color: "#555", letterSpacing: 1, display: "flex", alignItems: "center", gap: 6 }}>
                {sortOrder === "desc" ? "↓ Newest First" : "↑ Oldest First"}
              </button>
            </div>
            {filteredResults.length === 0 && <p style={{ textAlign: "center", color: "#bbb", fontSize: 15, marginTop: 40 }}>No results yet for this competition.</p>}
                ...
                return (
                  <div key={m.id}>
                    <button onClick={() => setSelectedMatch(isSelected ? null : m.id)}
                      ...
                      {m.motm && <span title="Man of the Match" style={{ fontSize: 12, color: "#888" }}>⭐ {m.motm}</span>}
                    ...
                    {isSelected && (
                      <div style={{ background: "#f8faff", borderRadius: "0 0 12px 12px", boxShadow: "0 4px 8px rgba(0,0,0,0.06)", overflow: "hidden" }}>
                        {/* Action buttons */}
                        <div style={{ display: "flex", gap: 0, borderTop: "1px solid #e8eeff" }}>
                          <button onClick={() => { setEditingResult({ ...m, scorers: m.scorers.join(", ") }); setEditOppLogo(null); }}
                            style={{ flex: 1, padding: "12px", background: "none", border: "none", borderRight: "1px solid #e8eeff", cursor: "pointer", fontFamily: "inherit", fontWeight: 800, fontSize: 13, color: "#1a1a2e", letterSpacing: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                            ✏️ Edit Result
                          </button>
                          <button onClick={() => { if (window.confirm(`Delete result vs ${m.opposition}?`)) handleDeleteResult(m.id); }}
                            style={{ flex: 1, padding: "12px", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", fontWeight: 800, fontSize: 13, color: "#d50000", letterSpacing: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                            🗑️ Delete
                          </button>
                        </div>
                        {/* Result card preview */}
                        <div style={{ padding: "16px" }}>
                          <ResultCard match={m} oppLogo={m.oppLogo || null} teamName={teamName} compColor={getCompColor(competitions, m.competition)} />
                        </div>
                      </div>
                    )}
                  </div>
            ...
            {editingResult && (
              <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
                <div style={{ background: "#fff", borderRadius: 16, padding: 24, width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 8px 40px rgba(0,0,0,0.2)" }}>
                  ...
                  {[
                    { label: "Opposition", key: "opposition", placeholder: "e.g. Houghton Juniors", type: "text" },
                    { label: "Leon Score", key: "homeScore", placeholder: "0", type: "number" },
                    { label: "Opposition Score", key: "awayScore", placeholder: "0", type: "number" },
                  ].map(field => (
                    <div key={field.key} style={{ marginBottom: 14 }}>
                      <label style={labelStyle}>{field.label}</label>
                      <input type={field.type} placeholder={field.placeholder} value={editingResult[field.key]}
                        onChange={e => setEditingResult(r => ({ ...r, [field.key]: e.target.value }))}
                        style={inputStyle} />
                    </div>
                  ))}
                  ...
                  <div style={{ marginBottom: 14 }}>
                    ...
                    <input type="text" placeholder="e.g. Grayson, Kayson ×2"
                      value={typeof editingResult.scorers === "string" ? editingResult.scorers : editingResult.scorers.join(", ")}
                      onChange={e => setEditingResult(r => ({ ...r, scorers: e.target.value }))}
                      style={inputStyle} />
                  </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
                    <div>
                      ...
                      <input type="text" placeholder="e.g. Grayson" value={editingResult.motm || ""}
                        onChange={e => setEditingResult(r => ({ ...r, motm: e.target.value }))}
                        style={inputStyle} />
                    </div>
                    <div>
                      <label style={{ ...labelStyle, color: "#aaa" }}>🏅 Opp. Man of Match</label>
                      <input type="text" placeholder="e.g. Smith" value={editingResult.oppMotm || ""}
                        onChange={e => setEditingResult(r => ({ ...r, oppMotm: e.target.value }))}
                        style={inputStyle} />
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    ...
                    <input ref={editFileRef} type="file" accept="image/*" onChange={e => {
                      const file = e.target.files[0]; if (!file) return;
                      const reader = new FileReader();
                      reader.onload = (ev) => setEditOppLogo(ev.target.result);
                      reader.readAsDataURL(file);
                    }} style={{ display: "none" }} />
                    <button onClick={() => editFileRef.current.click()}
                      ...
                      {(editOppLogo || editingResult.oppLogo)
                        ? <img src={editOppLogo || editingResult.oppLogo} style={{ width: 36, height: 36, objectFit: "contain" }} alt="opp logo" />
                        : "📁"}
                      {(editOppLogo || editingResult.oppLogo) ? "Logo uploaded ✓  (tap to change)" : "Upload opposition logo"}
                    </button>
                    {(editOppLogo || editingResult.oppLogo) && (
                      <button onClick={() => { setEditOppLogo("remove"); }}
                        style={{ marginTop: 6, background: "none", border: "none", color: "#d50000", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", letterSpacing: 1 }}>
                        ✕ Remove logo
                      </button>
                    )}
                  </div>
                  ...
                  <button onClick={handleSaveEdit}
                    style={{ width: "100%", padding: "15px", background: "#1a1a2e", color: "#87ceeb", border: "none", borderRadius: 12, fontSize: 16, fontWeight: 900, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", fontFamily: "inherit" }}>
                    Save Changes
                  </button>
                </div>
              </div>
        ...
        {mode === "scorers" && (
          <div style={{ maxWidth: 520, margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", background: "#fff", borderRadius: 12, overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.07)", marginBottom: 20 }}>
              {[{ key: "goals", label: "⚽ Goals" }, { key: "motm", label: "⭐ MOTM" }, { key: "oppmotm", label: "🏅 Opp MOTM" }].map(st => (
                <button key={st.key} onClick={() => setScorersTab(st.key)}
                  style={{ flex: 1, padding: "12px 6px", border: "none", background: scorersTab === st.key ? "#1a1a2e" : "none", color: scorersTab === st.key ? "#87ceeb" : "#999", fontWeight: 800, fontSize: 12, letterSpacing: 1, cursor: "pointer", fontFamily: "inherit", textTransform: "uppercase", transition: "all 0.15s" }}>
                  {st.label}
                </button>
              ))}
            </div>
            ...
            <div style={{ background: "#1a1a2e", borderRadius: 14, padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, boxShadow: "0 4px 16px rgba(0,0,0,0.12)" }}>
              {[
                { label: "Total Goals", val: filteredResults.reduce((a, r) => a + r.homeScore, 0) },
            ...
            {scorersTab === "motm" && <Leaderboard data={buildAwardBoard(filteredResults, "motm")} label="Man of the Match" emptyMsg="No MOTM awards recorded yet." filterLabel={filterComp === "All" ? "All Competitions" : filterComp} accentColor="#ffd700" />}
            {scorersTab === "oppmotm" && <Leaderboard data={buildAwardBoard(filteredResults, "oppMotm")} label="Opposition Man of the Match" emptyMsg="No Opp. MOTM awards recorded yet." filterLabel={filterComp === "All" ? "All Competitions" : filterComp} accentColor="#ff7eb3" />}
        ...
        {mode === "new" && (
          <div style={{ maxWidth: 520, margin: "0 auto" }}>
            {!newResult ? (
              <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
                    {competitions.map(comp => {
                      const active = form.competition === comp;
                      const cc = getCompColor(competitions, comp);
                      return (
                        <button key={comp} onClick={() => setForm(f => ({ ...f, competition: comp }))}
                          style={{ padding: "7px 14px", borderRadius: 20, border: active ? "none" : "1.5px solid #e0e0e0", background: active ? "#1a1a2e" : "#fff", color: active ? cc : "#888", fontWeight: 800, fontSize: 12, letterSpacing: 1, cursor: "pointer", fontFamily: "inherit", textTransform: "uppercase", transition: "all 0.15s" }}>
                          {comp}
                        </button>
                      );
                    })}
                    {!addingComp && (
                      <button onClick={() => setAddingComp(true)}
                        style={{ padding: "7px 14px", borderRadius: 20, border: "1.5px dashed #87ceeb", background: "#fff", color: "#87ceeb", fontWeight: 800, fontSize: 12, letterSpacing: 1, cursor: "pointer", fontFamily: "inherit" }}>
                        + New
                      </button>
                    )}
                  </div>
                  {addingComp && (
                    <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
                      <input autoFocus placeholder="Competition name..." value={newCompName} onChange={e => setNewCompName(e.target.value)}
                        onKeyDown={e => { if (e.key === "Enter") handleAddComp(); if (e.key === "Escape") setAddingComp(false); }}
                ...
                {[
                  { label: "Opposition", key: "opposition", placeholder: "e.g. Houghton Juniors", type: "text" },
                  { label: "Leon Score", key: "homeScore", placeholder: "e.g. 3", type: "number" },
                  { label: "Opposition Score", key: "awayScore", placeholder: "e.g. 1", type: "number" },
                ].map(field => (
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <button onClick={() => fileRef.current.click()}
                    ...
                    {oppLogo ? <img src={oppLogo} style={{ width: 36, height: 36, objectFit: "contain" }} alt="opp" /> : "📁"}
              ...
              <div>
                <ResultCard match={newResult} oppLogo={newResult.oppLogo || oppLogo} teamName={teamName} compColor={getCompColor(competitions, newResult.competition)} />
                ...
                <p style={{ textAlign: "center", color: "#aaa", fontSize: 13, marginTop: 12 }}>Tap Save to download as an image ready for Facebook 📘</p>
              </div>
            )}
          </div>
        )}
